...
if(isset($_FILES["name"])){
	...
	if($up->uploaded){
		...
		if($up->processed){
            ...
            $highestColumn = $sheet->getHighestColumn();
            for ($row = 2; $row <= $highestRow; $row++){ 
                $x_CURP = $sheet->getCell("A".$row)->getValue();
                $x_NOMBRE = $sheet->getCell("B".$row)->getValue();
                $x_APELLIDO = $sheet->getCell("C".$row)->getValue();
                $x_DOMICILIO = $sheet->getCell("D".$row)->getValue();
                $x_EMAIL = $sheet->getCell("E".$row)->getValue();
                $x_TELEFONO = $sheet->getCell("F".$row)->getValue();
                $sql = "insert into person (CURP, NOMBRE, APELLIDO, DOMICILIO, EMAIL, TELEFONO) value ";
                $sql .= " (\"$x_CURP\",\"$x_NOMBRE\",\"$x_APELLIDO\",\"$x_DOMICILIO\",\"$x_EMAIL\",\"$x_TELEFONO\", NOW())";
               $con->query($sql);
            }
    	unlink($archivo);
        }			
