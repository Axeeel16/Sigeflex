...
<div class="container">
		<header class="he  bg-light">
				 <h5 class="titu_head_bienv">Reportes de Alumnos</h5>
 ...
 if($id=="Viven")
	{
		 ...
		 $consulta = ("SELECT *,COUNT(localidad) AS Cantidad FROM usuarios WHERE localidad = 'Grand Bourg'");
		...
		echo '<table class="table table-striped">';
		 ...
		 echo"<th>Id_usuario</th> <th>DNI</th> <th>Nombre</th> <th>Apellido</th> <th>Dir</th> <th>Localidad</th> <th>Tel</th> <th>Cel</th> <th>Usuario</th> <th>Contraseña</th> <th>Edad</th> <th>Fecha de Nacimiento</th> <th>Cantidad</th>";
	...
	}else  if($id=="Cant_Mayores")
		{ 
		 $consulta = ("SELECT *,COUNT(edad) AS Cantidad FROM usuarios WHERE edad >= 18");
		  $mysql= new mysqli("localhost", "root", "","bd_usuario");
		if($mysql->connect_error)
		die("Error de conexión");
	 ...
	 echo "</table>";
		...
		{ 
		 $consulta = ("SELECT *, COUNT(*) AS Cantidad FROM `usuarios` WHERE `fechaNac`<'2008-01-01'");
		  $mysql= new mysqli("localhost", "root", "","bd_usuario");
		if($mysql->connect_error)
		die("Error de conexión");
